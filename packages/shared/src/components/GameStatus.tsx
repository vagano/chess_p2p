...
export function GameStatus({ gameState, game, playerColor, isMyTurn, isCheck, isPending }: GameStatusProps) {
  const renderStatus = () => {
    ...
    if (isPending) {
      return <span style={{ color: '#ff9800' }}>Confirming move...</span>;
    }
  ...
  const renderMoveList = () => {
    ...
    return (
      <div style={{ maxHeight: '180px', overflowY: 'auto', fontSize: '12px', fontFamily: "'SF Mono', 'Fira Code', monospace" }}>
        {pairs.map((pair) => (
          <div
            key={pair.num}
            style={{
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', width: '100%' }}>
      ...
      <div style={card}>
        <div style={{
          fontSize: '11px', fontWeight: 700, color: 'var(--tg-theme-hint-color, #888)',
      ...
      {gameState.moveError && (
        <div style={{ ...card, background: 'rgba(244,67,54,0.15)', color: '#f44336', fontSize: '13px' }}>
          Move rejected: {gameState.moveError}
        </div>
      )}
    </div>
  );
